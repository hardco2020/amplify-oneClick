...
TABLE_NAME = "Event-" + os.environ["ENV"]
...
def handler(event, context):
    try:
        ...
        for item in items:
            print(item)
            result = post_process_hit(
                item["payload"], type, s3uri, images_prefix, labels_prefix
            )
        return {
            "statusCode": 200,
            "headers": {
...
def post_process_hit(payload, type, s3uri, images_prefix, labels_prefix):
    try:
        ...
        if first == -1:
            ...
            image_s3key = (
                s3uri[first + 1 :] + "/" + images_prefix + "/" + image_file_name
            )
        ...
        if first == -1:
            ...
            label_s3key = (
                s3uri[first + 1 :] + "/" + labels_prefix + "/" + label_file_name
            )
